...
function Registration() {
  ...
  const handleSubmit = async () => {
    // setLoading(true);
    const payloadRegistration = { userName, userPassword, };
    console.log("payload", payloadRegistration);
    const registrationRes = await axios
      .post(API_URL, payloadRegistration, {
        headers: { "Content-Type": "application/json" },
      })
      .catch((error) => setError(error.message))
      .finally(() => setLoading(false));
    console.log("responses", registrationRes)
    localStorage.setItem("auth-token", registrationRes.data.data.token);
          ...
          <div className="mt-10 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
            ...
            <div className="mt-12">
              ...
              <div className="mt-10 text-sm font-display font-semibold text-gray-700 text-center">
                Sudah punya akun?{" "}
                <a className="cursor-pointer text-red-500 hover:text-red-600" onChange={handleOnclick}>
                  Langsung Login!
                </a>
              </div>
